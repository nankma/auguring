...
6. **The single-turn "add X to my interests" fast path is retired.**
   ...
   whether this conversation keeps going without converging -- nothing
   unsafe can happen while it does, and (2026-09-24, see
   docs/plans/front-door-agent-plan.md) there is no more end_exploration
   tool or turn ceiling to enforce that with: this conversational agent is
   now the one always-on front door for every on-topic message, so
   "nothing unsafe can happen" no longer needs a bound on how long any
   particular conversational mode lingers -- there is no separate mode
   left to linger in. This is also why set_interest/remove_interest/
   set_language moved into this same agent instead of a one-shot
   deterministic dispatch: a subscriber naming a topic outright ("add
   robotics") still needs to see it grounded before it's saved, and
   switching languages or removing an interest needs to keep working
   without forcing them out of an in-progress exploration first.
...
_SYSTEM_PROMPT = (
    ...
    "WHEN TO STOP TRYING: if they are satisfied, say they are done, or "
    "you have gone in circles without converging, just say so plainly -- "
    "there is no tool to call for this, it is simply what you say next. "
    "If you have gone in circles, say honestly that this isn't getting "
    "anywhere and suggest they name a company or topic directly instead "
    "of continuing to guess. This does not close or reset anything -- if "
    "they come back with something new afterward, treat it exactly like "
    "any other fresh request, in the same conversation.\n\n"
...
def _session(runtime: ToolRuntime) -> dict:
    """A fresh, per-turn dict the caller (bot.py) seeds with the
    conversation's current pending_proposal (if any) and reads back
    afterward -- the only state that crosses turns now that there's no
    separate exploration "session" to track (docs/plans/front-door-agent-plan.md).
    The propose_*/save_*/drop_* tools below write into it so the caller
    can see what happened without parsing the agent's message
    transcript."""
...
def execute_save(chat_id: int, topic: str, definition: str, guard_model) -> str:
    ...
    # The one outcome that says this feature worked -- without this there
    # is no way to ask "how many conversations actually produce an
    # interest", which is the only number that matters here.
    _events.log("interest_saved_from_exploration",
                 {"message": f"exploration saved an interest: {topic}", "chat_id": chat_id, "topic": topic})
    return reply


def execute_drop(chat_id: int, topic: str) -> str:
...
def execute_redefine(chat_id: int, topic: str, definition: str) -> str:
...
def save_interest(topic: str, definition: str, runtime: ToolRuntime) -> str:
    ...
    return execute_save(runtime.context["chat_id"], topic, definition, runtime.context.get("guard_model"))
...
def drop_interest(topic: str, runtime: ToolRuntime) -> str:
    ...
    return execute_drop(runtime.context["chat_id"], topic)
...
def save_definition(topic: str, definition: str, runtime: ToolRuntime) -> str:
    ...
    return execute_redefine(runtime.context["chat_id"], topic, definition)
...
class _BareConfirmationCheck(BaseModel):
    reasoning: str
    is_bare_confirmation: bool


_BARE_CONFIRMATION_PROMPT = (
    "A subscriber sent the following message to a technology-news bot, and "
    "there is NO pending question or proposal on record for this "
    "conversation right now. In the `reasoning` field, briefly explain your "
    "answer, then decide: does this message ONLY make sense as answering a "
    "yes/no or multiple-choice question the bot must have just asked -- e.g. "
    "\"yes\", \"sure\", \"go ahead\", \"the second one\", \"no thanks\" -- "
    "naming no topic, company, or subject of its own? Answer false for "
    "anything that stands on its own, including a full sentence, a named "
    "topic, or a question, even a short one."
)


def reads_as_bare_confirmation(guard_model, user_text: str) -> bool:
    """True if `user_text` only makes sense as an answer to a pending
    offer -- checked ONLY when the caller has BOTH no pending offer AND no
    conversation history at all for this chat (see bot.py's
    _process_agent_turn). With real history, the top-level agent can
    resolve an ordinary contextual follow-up itself; this is only for the
    case where there is nothing left to resolve it against. Catches the
    orphaned-confirmation defect docs/plans/front-door-agent-plan.md
    documents: history and a pending offer used to have different
    lifetimes (a session could close in the same turn a question was
    asked, while the conversation history survived), so a later "yes"
    could answer a question nothing remembers asking any more. Rather than
    let that fall through into the agent and get guessed at, the caller
    replies honestly that the context is gone.

    Same shape and reliability class as classify_confirmation/
    guardrails.classify_message -- a small, bounded classification call,
    not a new open-ended loop. Fails open to False (treat it as an
    ordinary self-contained message) so a classifier hiccup never blocks
    a real request -- the cost of a false negative here is just the
    normal pipeline running, not silence."""
    if guard_model is None:
        return False
    try:
        structured = guard_model.with_structured_output(_BareConfirmationCheck, method="function_calling")
        result = structured.invoke([
            {"role": "system", "content": _BARE_CONFIRMATION_PROMPT},
            {"role": "user", "content": user_text},
        ])
        if result is None:
            return False
        return result.is_bare_confirmation
    except Exception as exc:
        _events.log("bare_confirmation_check_failed", {"message": "orphaned-confirmation check failed"},
                     level=Level.WARN, exc=exc)
        return False
...
def run_turn(chat_id: int, user_text: str, history: list, session: dict,
    ...
    `session` is a fresh per-turn dict the caller seeds with the
    conversation's current pending_proposal (if any) -- see `_session`'s
    own docstring. There is no more "done" signal to read back: this
    agent is the one always-on front door for every on-topic message now
    (docs/plans/front-door-agent-plan.md), so a turn ending never closes
    ...
    try:
        ...
        # Hitting the step ceiling is a bounded outcome, not a crash --
        # say something honest instead of letting the caller's generic
        # handler send LangGraph's own error text -- "Recursion limit of
        ...
        _events.log("interest_turn_out_of_steps",
                     {"message": "one turn hit the step ceiling",
...
def out_of_steps_message() -> str:
    ...
    a single turn spent itself searching. Measured cause, 2026-09-08: a
    topic with no coverage at all, which the model kept rephrasing. So
    this says that plainly rather than blaming the conversation."""
    return (
