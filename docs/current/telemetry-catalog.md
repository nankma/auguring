...
| `router_failed` | `argus.guardrails` | `guardrails.classify_message` (layer 2, now Jev -- see item 5 of docs/plans/front-door-agent-plan.md) | **ERROR** | — | load-bearing: silent fail-open here is the exact 2026-08-21 incident (`docs/plans/guardrails-plan.md`) — don't let a future alert audit downgrade this to WARN |
| `output_check_failed` | `argus.guardrails` | `guardrails.is_output_on_topic` (layer 4, now Jev) | **ERROR** | — | same reasoning as `router_failed`, its layer-4 mirror |
| `router_no_categories` | `argus.guardrails` | `guardrails.classify_message` (layer 2) | WARN | `user_message` | on-topic but no category question came back true -- shouldn't happen, and self-correcting (handled as a news query), so a rate to watch rather than a page. Was a bare `print()` until a 2026-09-24 code review pointed out it was the one anomaly in this module not queryable in production |
| `incomplete_reply` | `argus.guardrails` | `guardrails.is_output_on_topic` (layer 4's new all_asks_addressed question) | WARN | `user_text`, `bot_reply` | observability-only, NOT enforced -- a multi-intent reply that only satisfies some of what was asked, measured live (qa-engineer, 2026-09-24) at ~12% for one representative phrasing. Only fires when the caller passes `user_text` (skipped for a reply with no meaningful "ask" to check completeness against, e.g. `bot._lost_context_reply`'s translated apology) |
