...
docs/plans/telemetry-and-testing-plan.md). Layers 2 and 4 run on Jev
since docs/plans/front-door-agent-plan.md item 5 -- OPENROUTER_API_KEY
(or settings.yml's jev.api-key pointing at it), not DEEPSEEK_API_KEY.
Run manually:
...
LAYER2_CASES = [
    ...
    #
    # No more expects_topic/expected_push_interval_hours/
    # expected_language_contains fields (removed 2026-09-24 alongside the
    # move to Jev, docs/plans/front-door-agent-plan.md item 5) -- Jev is a
    # typed-decision model, not a free-text extractor, and classify_message
    # no longer extracts topics/an interval/a language at all (nothing has
    # read those fields for dispatch since Step B anyway). Only on_topic
    # and category are measured now.
    ...
    # Multi-topic set_interest -- the 2026-08-25 bug this dataset originally
    # caught a regression of (a single `topic: str` field forcing several
    # named topics through one garbled label). The topic-extraction half of
    # that check is gone along with the other extraction fields (Jev
    # doesn't do free-text extraction); on_topic/category classification
    # for these multi-topic phrasings is still worth its own coverage.
    {"text": "Add AI agent, AI coding, and LLM to my interests", "on_topic": True,
     "category": "set_interest", "group": "multi_topic_set_interest"},
    {"text": "我想追蹤機器人科技、半導體、還有光通訊", "on_topic": True,
     "category": "set_interest", "group": "multi_topic_set_interest"},
    {"text": "Remove crypto and robotics from my interests", "on_topic": True,
     "category": "remove_interest", "group": "multi_topic_set_interest"},
    ...
    {"text": "Add robotics to my interests", "on_topic": True, "category": "set_interest",
     "group": "find_interests_vs_set_interest"},
...
# Category checks are SETS, not ordered lists -- since the move to Jev
# (docs/plans/front-door-agent-plan.md item 5), multi-category support
# comes from asking one independent yes/no question per category, and
# `categories` is built by iterating those in a fixed dict-definition
# order that carries no "this is the order to address them in" meaning
# any more. Two flavors, not one, and the distinction matters:
# `expected_categories_exact` (categories must equal this set exactly) is
# for cases with one genuinely correct answer, INCLUDING the negative
# controls below -- a subset check would never catch over-splitting,
# since "extra categories present" always passes a subset check.
# `expected_categories_containing` (categories must be a superset) is
# only for a message ambiguous enough that trial-to-trial variation in
# which extra intents fire is itself the expected finding, not a bug.
LAYER2_MULTI_INTENT_CASES = [
    {
        "text": "Add robotics to my interests and tell me what's new with it",
        "on_topic": True,
        "expected_categories_exact": {"set_interest", "news_query"},
    ...
    },
    {
        ...
        "on_topic": True,
        "expected_categories_exact": {"start_push", "set_language"},
        "group": "english_multi_intent",
    },
    ...
    # its own regression, not caught by the positive cases above -- must
    # use expected_categories_exact, not _containing, for exactly that
    # reason (see this dataset's own module comment).
    {
        "text": "What's new with OpenAI's latest model release this week?",
        "on_topic": True,
        "expected_categories_exact": {"news_query"},
        "group": "single_intent_control",
    },
    {
        "text": "Turn on push every 6 hours",
        "on_topic": True,
        "expected_categories_exact": {"start_push"},
        "group": "single_intent_control",
    },
    # A real subscriber's phrasing (see the SHioufen trace investigation
    ...
    # inspecting 10 raw trials (pre-Jev) showed the model reading this as
    # genuinely 2-3 intents (start_push always; set_language in 9/10;
    # set_interest, for the "tech/finance/bitcoin" qualifier, in 8/10), not
    # a stable fixed set. expected_categories_containing checks the two
    # unambiguous intents are always present (extras-tolerant) rather than
    # pinning an exact answer this sentence doesn't actually have one true
    # version of.
    {
...
# is_output_on_topic(response_text, jev_api_key) -> bool (True = allow),
# via Jev since docs/plans/front-door-agent-plan.md item 5. Each case
# still carries its own `category` label, kept for the report only (which
# kind of reply this is) -- it doesn't affect the check itself; every
# reply gets the same full check now that Route B's fixed-template narrow
...
def measure_layer2(jev_api_key: str, cases: list[dict], trials: int) -> list[dict]:
    ...
    for case in cases:
        ...
        for _ in range(trials):
            classification = guardrails.classify_message(case["text"], jev_api_key)
            correct = (
...
def _categories_correct(case: dict, classification) -> bool:
    """`expected_categories_exact` requires an exact set match -- use it
    when a message has one genuinely correct answer, INCLUDING a negative
    control where over-splitting must be caught (a subset check would
    never catch that, since extra categories always pass one).
    `expected_categories_containing` only requires a superset -- use it
    for a message ambiguous enough that trial-to-trial variation in which
    extra intents fire is itself the finding (see LAYER2_MULTI_INTENT_CASES'
    mixed_language_control case)."""
    actual = set(classification.categories)
    if "expected_categories_exact" in case:
        return actual == case["expected_categories_exact"]
    if "expected_categories_containing" in case:
        return case["expected_categories_containing"].issubset(actual)
    return True


def measure_layer2_multi_intent(jev_api_key: str, cases: list[dict], trials: int) -> list[dict]:
    """Same shape as measure_layer2, but for LAYER2_MULTI_INTENT_CASES --
    checks `categories` against expected_categories_exact/
    expected_categories_containing (see _categories_correct), not a
    single value."""
    results = []
    for case in cases:
        trial_results = []
...
def measure_layer4(jev_api_key: str, cases: list[dict], trials: int) -> list[dict]:
    """Measures only the two original checks (discusses_own_configuration,
    appropriate_bot_content) -- this dataset has no paired "what did the
    user ask" for each reply, so the newer all_asks_addressed completion
    question (docs/plans/front-door-agent-plan.md) is left at its default
    (skipped) by not passing user_text. See that question's own docstring
    in guardrails.py for why it's observability-only anyway, not something
    this pass/fail measurement would act on differently."""
    results = []
    for case in cases:
        trial_results = []
        for _ in range(trials):
            actual = guardrails.is_output_on_topic(case["text"], jev_api_key)
...
def print_layer2_multi_intent_report(results: list[dict]) -> None:
    _print_ntrial_report(
        ...
        results,
        lambda r: f"{r['on_topic']}/{sorted(r.get('expected_categories_exact') or r['expected_categories_containing'])}",
    )
...
def main():
    ...
    if args.layer in ("2", "4", "all"):
        # Jev, not a models.* entry (docs/plans/front-door-agent-plan.md
        # item 5) -- to measure a candidate provider/key instead, point
        # SETTINGS_FILE at a settings.yml whose jev.api-key names it, then
        # re-run and compare the report against a prior run's.
        try:
            jev_api_key = get_settings().resolved("jev.api-key", required=True)
        except SettingsError as exc:
            print(f"\njev.api-key not configured -- skipping layers 2/4 (need a real Jev call): {exc}",
                  file=sys.stderr)
            return
        if args.layer in ("2", "all"):
            print_layer2_report(measure_layer2(jev_api_key, filtered(LAYER2_CASES), args.trials))
            print_layer2_multi_intent_report(
                measure_layer2_multi_intent(jev_api_key, filtered(LAYER2_MULTI_INTENT_CASES), args.trials)
            )
        if args.layer in ("4", "all"):
            print_layer4_report(measure_layer4(jev_api_key, filtered(LAYER4_CASES), args.trials))
